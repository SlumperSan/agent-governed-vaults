...
test('an unreadable aggregator() and phaseId() do not become "answered neither"', async () => {
  // Since the swap-key split, the identity/swap leg is `swapNotice()` — the SECOND result — while
  // the FIRST is the harm finding, which still passes (desc/decimals were not touched here).
  const reader = readerFor({ overrides: { [FEED]: { aggregator: UNREACHABLE, phaseId: UNREACHABLE } } });
  const [harm, r] = await run(reader);
  assert.equal(harm.status, 'ok', 'the harm legs ran and passed; only the identity/swap leg is blind');
  assert.equal(r.detail.detectorBroken, true);
  assert.equal(r.key, `${ASSET}:swap`);
