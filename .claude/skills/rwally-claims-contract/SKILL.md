...
`apps/site/test/site.test.mjs` (36 tests, pinning wording). A redesign does not relax any of it: the
guards run against **build output**, so a React component that renders a banned shape reds CI the
same as a hand-written page.
...
- **A blanket negative about the operator.** Never "the operator has no power / no privileged
  role / no on-chain authority". The operator is the sole recipient of the 10% performance fee
  (`FeeEngine.onFeeCollected` → `claimableFees[operatorAddressOf(opId)]`), so any universal is
  falsifiable in one transaction. Write the enumeration: *operatorship confers no authority to
