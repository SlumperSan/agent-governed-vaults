...
| # | Item | Why it matters | Status |
|---|---|---|---|
| 4 | **`builder-code` referral tracking** | 2,670 routes (17.2%) confirmed carry `extensions['builder-code'].info.a` (only `a` ever populates — `s`/`w` unused in a static catalog per `docs.x402.org/extensions/builder-code`). `row_from_record` now captures the full object going forward; `backfill_referral_json.py` ready to backfill existing rows (8,107) + seed `referral_event` (2,670, no dupes) — tested only against copies, **NOT yet run against production `x402_index.db`** (still sweep-locked; confirmed 0 rows populated there as of this check). Revenue-share to builder-code holders: NOT ESTABLISHED (Base frames it as protocol-discretionary). See `notes-builder-code.md`. Next: run backfill once the sweep releases the write lock. | researched, ready to apply |
...
| 11 | **Fix `v_builder_codes`/`v_latest_catalog` to filter `is_complete`** | Surfaced by P1#4 rework: neither view filters incomplete snapshots, which would silently corrupt any read API (#6) or referral dashboard built on top. Small fix, must land before #6 touches these views. | open |
