...
contract AuditAggregatorSwapDriftTest is Test {
    ...
    // binding on it. What this suite pins is the ARITHMETIC at these values -- which drifts the
    // band catches and which it does not. The bindings live elsewhere, in two layers:
    // `ChainlinkOracle`'s own MAX_BAND_RATIO (a legal band can never let a 2-decimal drift escape
    // in BOTH directions), and `scripts/verify-chainlink-oracle.mjs`'s `band bounds a 2-decimal
    // drift`, which checks the live price actually sits in that window rather than near an edge.
