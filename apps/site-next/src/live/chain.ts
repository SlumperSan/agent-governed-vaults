...
/*
 * A THIRD ADDRESS CONSTANT WAS DELETED HERE ON 2026-09-09, AND THE DELETION IS
 * THE RECORD.
 *
 * It held the token launched through a third-party launchpad on 2026-09-05, it
 * was rendered as the hero's third copy chip, and it was never called by
 * anything on this page. The owner retired that launch on 2026-09-09, calling
 * it a test, and ordered every token sentence off every public surface until a
 * real relaunch.
 *
 * IT WAS DELETED RATHER THAN LEFT UNUSED, and the reason is specific to this
 * file. An exported string constant is not a comment: an unreferenced export in
 * a module the page already imports is one accidental reference away from being
 * in a shipped bundle again, and a reader who finds an address in the JavaScript
 * has been told something whether or not a paragraph says it. The retired
 * launch's own record survives at
 * `contracts/config/deployments/rwly-robinhood-mainnet.json`, which is where
 * history belongs; a page constant is not history, it is publication.
 */
