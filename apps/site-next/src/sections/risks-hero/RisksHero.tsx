 ...
 * cannot be made to carry it there instead), the licence sentence, the
 * jurisdiction/geofencing paragraph, and the total-loss paragraph.
 * FOOTER_LICENCE is imported from pinned.ts rather than retyped, for the
 * byte-identity reason that file's header gives.
 *
 * THE TOKEN SENTENCES ARE GONE FROM THIS FILE. A `No token.` sentence was
 * retired from it on 2026-09-05 when a launch made the clause false, and the
 * four-paragraph token block that replaced it was itself removed on 2026-09-09
 * when the owner retired that launch as a test. The long note where those
 * paragraphs used to stand says what each of them carried and why removal
 * rather than rewording was the right move. One consequence constrains any
 * replacement copy rather than merely recording history: the retired `No
 * token.` sentence held this site's only exemption for the two negated words it
 * carried, so nothing written into this hero may reach for either of them.
 */
...
/* ---------------------------------------------------------------------------
 * THE TOKEN BLOCK WAS REMOVED ON 2026-09-09, AND THE REMOVAL IS THE RECORD.
 *
 * WHAT WAS HERE. Four paragraphs added on 2026-09-05: what the token was and at
 * which address, how the launchpad curve charged and where the creator fee
 * rights sat, what the protocol's own deployer address took in the launch
 * transaction, and which parts of the design around it were intent rather than
 * code. Every figure in them was an on-chain read.
 *
 * WHY THEY WENT. The owner retired that launch on 2026-09-09, calling it a test,
 * and ordered every token sentence off every public surface until a real
 * relaunch. That is not a claims correction: none of those paragraphs was false
 * when it was written and none of them became false. It is an editorial
 * decision that the site does not describe a retired test launch at all, and a
 * disclosure block about a thing the site no longer mentions is a token beat
 * with a disclaimer's typography.
 *
 * WHAT DID NOT GO WITH THEM, because the two are easy to confuse. Every risk on
 * this page that is about the PROTOCOL stayed: the deployment paragraph, the
 * not-an-offer sentence, the licence, the jurisdiction and geofencing
 * paragraph, the total-loss paragraph, and every row of the register. Only the
 * paragraphs whose subject was the token were removed.
 *
 * WHY THE CONSTANTS WERE DELETED RATHER THAN LEFT UNRENDERED. An unrendered
 * string constant in a section file is a paragraph one JSX line away from being
 * published again, and the removal would then be invisible in the diff that
 * republished it. `scripts/test/claims-token-absence.test.mjs` walks the built
 * pages and reds if any of this comes back, but a guard is the second line and
 * not the first.
 *
 * THE RECORD OF THE RETIRED LAUNCH SURVIVES at
 * `contracts/config/deployments/rwly-robinhood-mainnet.json`, whose own
 * `status` field says the launch was a test, that it is kept as history, and
 * that no public surface cites it. Nothing was destroyed here; it stopped being
 * published.
 * ------------------------------------------------------------------------ */
