...
t('the corrections from the 2026-08-29 review have not been undone', () => {
  for (const p of PAGES) {
    ...
    // that returns it. Anything else is an unsourced figure. The same leg in
    // apps/site/test/site.test.mjs carries the same rule; the two move together.
 ...
 *
 * WHAT STOOD HERE. A window leg written on 2026-09-05, the evening the token was created, and
 * rewritten the same evening: it required the address stem `0x2eed8ae7`, the words `fixed supply`,
 * or a design-intent hedge within 160 characters of every mention of the ticker; a companion check
 * on the terms that named things with no on-chain existence at all; a purchase-vocabulary ban; a
 * floor of ten mentions so the window rule could not be satisfied by deleting the copy; a per-page
 * list of launch facts each page had to STATE; and a probe proving the window bit and spared the
 * three approved forms.
 *
 * WHY IT WENT. On 2026-09-09 the owner retired that launch, calling it a test, and ordered every
 * token sentence off every public surface until a real relaunch. The floor and the must-state list
 * are POSITIVE requirements to publish token facts, and they cannot coexist with an order not to
 * publish token facts. The window rule and the probe would have survived only as vacuous truths
 * over pages with nothing left to match, which is the failure mode this file's own header spends
 * three paragraphs warning about.
 *
 * WHAT REPLACES IT. `scripts/test/claims-token-absence.test.mjs`, a repository guard rather than a
 * page guard, because the property is now repository-wide rather than per-page: no built page of
 * this build, none of the nine corpus pages, none of the three llms.txt copies and no built page of
 * apps/app may name the token, its address, the launchpad, the curve or a supply figure. That file
 * asserts a minimum file count per surface group BEFORE it asserts absence, and carries a probe
 * proving the ban bites on every banned shape and spares `RWAlly`, the project's own name.
 *
 * THE PURCHASE-VOCABULARY BAN WENT WITH THE REST, and that is worth one sentence of justification
 * rather than being passed over: every one of its patterns (`buy RWLY`, `buy the token`, `how to
 * buy`, `where to buy`) is a phrase about a token, and a page that may not name a token cannot
 * carry one. The absence rule reds on the noun before the verb becomes reachable.
 */
...
/*
 * RE-MEASURED 2026-09-09, AFTER THE CLOSING BEAT WAS DELETED, AND THE FLOOR DID NOT MOVE.
 *
 * `sections/index-next` was removed that day with the rest of the token copy, taking a heading and
 * a 49-word paragraph off the page. The obvious next move is to lower the floor to match, and it
 * would have been the wrong one: the page measures 191 visible words without the beat, which is 41
 * clear of the floor. A floor lowered to accommodate a deletion that did not breach it is a floor
 * that has stopped meaning anything, and the number recorded here is the measurement rather than
 * the permission.
 *
 * The floor's own reasoning is unchanged and is worth restating because it is what makes the
 * measurement matter: below 150 words this page has stopped saying what the protocol is, and the
 * failure it guards against is a slow one, a sentence at a time, each removal individually
 * defensible. If a future edit does take the page under 150, the answer is to restore protocol copy
 * the corpus already carries, not to move this number.
 */
...
/*
 * SOURCE 4 WAS REMOVED ON 2026-09-09, AND REMOVING A SOURCE IS A TIGHTENING.
 *
 * WHAT IT WAS. `RWLY_LAUNCH_RECORD`, four sentences of on-chain readings of the 2026-09-05 launch,
 * pinned here because the corpus was written before the token existed and so could not contain a
 * true sentence about it. Every clause was tagged in `rwly-flip-facts.md` and its companion
 * `rwly-robinhood-mainnet.json` with the `cast` call that established it.
 *
 * WHY IT WENT. The owner retired that launch on 2026-09-09 as a test and ordered every token
 * sentence off every public surface. The homepage beat those four sentences existed to source,
 * `sections/index-next`, was deleted in the same change, so the source has nothing left to source.
 * A provenance list with an entry nothing quotes is a standing invitation to write a new sentence
 * that happens to match it, which is the exact failure the note on source 3 warns about.
 *
 * THE HAYSTACK IS NOW THREE SOURCES, which means the homepage's sentences must come from the
 * corpus, the approved promo script, or the small set of strings that cannot exist in either. That
 * is where this leg started before a launch forced a fourth, and it is a stricter test than the one
 * it replaces.
 */
