...
2. **Even-`k` average-of-two-middle** (`:87`):
   `k % 2 == 1 ? fresh[k/2] : (fresh[k/2-1] + fresh[k/2]) / 2`. At small `k` this is fragile.
   A single source at an extreme drags the average by ~half its deviation (e.g. a Chainlink feed
