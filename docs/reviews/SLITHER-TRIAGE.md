...
`protocol/main` @ `bab5ee90`, pinned to ±1% of `NAV_GAS_MEASURED = 10_402_702` and additionally
...
shortfall loop; `_childPendingExecution` (`VaultCore._settleExit:664`) and the child share read
(`VaultCore._settleExit:668`) read child views in the same loop.
...
`Governance.execute` writes `p.status = Status.Executed` at `Governance.execute:595` and makes the
external `executeRebalance` call at `Governance.execute:611` — *after* the write. An out-of-gas inside the loop
therefore reverts the whole transaction and unwinds the status write, leaving the proposal `Passed`.
`hasPendingExecution` then returns `block.timestamp <= p.expiresAt` (`Governance.hasPendingExecution:651`), which keeps the
