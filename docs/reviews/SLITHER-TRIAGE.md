...
| …and the three `block.timestamp` WRITES into `proposals` / `standingDefaultOf` (`Governance.propose:293`, `Governance.setStandingDefault:452`, `Governance.finalize:574`) also replaced | **17** |
...
| --- | --- | --- | --- |
| `p.commitDeadline` | `Governance.commitVote:358` `<` | `Governance.revealVote:375` / `Governance.revealDelegated:405` / `Governance.applyStandingDefault:478` `>=`; `Governance.hasPendingExecution:648` `>=` | commit closed, reveal open, Mode F on |
| `p.revealDeadline` | reveal family `<` — `Governance.revealVote:376`, `Governance.revealDelegated:406`, `Governance.applyStandingDefault:479` | `Governance.finalize:529` `>=` | reveal closed, finalize open |
| `p.expiresAt` | `Governance.execute:592` `<=`; `Governance.hasPendingExecution:651` `<=` | `Governance.markExpired:620` `>`; `Governance._refreshStatus:627` `>` | execute open, Mode F on, expiry closed |
| `p.createdAt` | `d.setAt < createdAt` strict — `Governance.applyStandingDefault:491` | snapshots read `createdAt - 1` — `Governance.propose:297`, `:298`, `:314`, `Governance._boundedWeight:348` | same-second default rejected, same-second stake weightless — same strictness on both sides |
...
Staleness math cannot underflow or wrap. `ChainlinkOracle.priceWad:290` rejects a future
`updatedAt` before `:293` subtracts, and `:293` saturates anyway (`block.timestamp > cfg.heartbeat
? block.timestamp - cfg.heartbeat : 0`); `ChainlinkOracle._requireSequencerUp:326` rejects a future
`startedAt` before `:328` subtracts; `VaultCore._mintShares:491` is the only writer of
`lastDepositTime` (`= block.timestamp`) so `VaultCore._exitFeeBps:1007` can never go negative. Every governance deadline addend is a `uint32` bounded by
...
**Row 6 — `Governance.applyStandingDefault:490-492`.** See T-1 below. Slither
...
| --- | --- | --- | --- | --- |
| 1 | `Governance.propose` `:291` | `block.timestamp >= lastAt + cfg.proposalCooldown` — whether this proposer may open another proposal | **DESIGN** | M-7 rate limit, bounded to `[1h, 30d]` by `_validateConfig`. Boundary now pinned by `AuditGovernanceDeadlineBoundaries::test_proposalCooldown_boundary`. Honest residual already recorded at `Governance._validateConfig:265-267`: `lastProposalAt` is per-PROPOSER, so a second address sidesteps it. That is not a timestamp defect. |
| 2 | `ChainlinkOracle.priceWad` `:290, :293, :294` | whether a feed round is fresh: reject future stamps, then `updatedAt >= block.timestamp - heartbeat` | **DESIGN** | Fail-closed staleness (C-6). Exactly-at-heartbeat is fresh; `:293` saturates so a heartbeat above the clock cannot underflow-panic out of `StaleOracle`. `AuditTimestampBoundaries::test_heartbeat_partitionsExactly`, plus existing `ChainlinkOracle::test_freshAtHeartbeatBoundary` / `::test_failClosed_stale`. |
| 3 | `ChainlinkOracle._requireSequencerUp` `:326, :328` | whether the L2 sequencer has been up longer than `GRACE_PERIOD` | **DESIGN** | Conservative direction: exactly-at-grace reads DOWN, so the window must fully elapse. `:316` guards `:318` against underflow. Already the best-covered family in the suite — `ChainlinkOracleSequencerFork` exercises grace−1, grace and grace+1. |
| 4 | `Governance.revealVote` `:375-376` | reveal-phase membership `[commitDeadline, revealDeadline)` | **DESIGN** | Exact complement of `Governance.commitVote:358` and `Governance.finalize:529`. `AuditGovernanceDeadlineBoundaries::test_commitDeadline_partitionsExactly` and `::test_revealDeadline_partitionsExactly`. |
| 5 | `Governance.markExpired` `:620` | `block.timestamp > p.expiresAt` — whether a passed proposal may be marked Expired | **DESIGN** | The strict `>` is the exact complement of `execute`'s `<=` (`:571`). `AuditGovernanceDeadlineBoundaries::test_expiresAt_partitionsExactly_executeVsMarkExpired`. |
| 6 | `Governance.applyStandingDefault` `:478-479` (phase) and **`:491` (TTL)** | the phase guard is fine; **the TTL guard `block.timestamp <= d.setAt + DEFAULT_TTL` is evaluated during the REVEAL phase, so a standing default's usable life is `DEFAULT_TTL - cfg.commitDuration`, not `DEFAULT_TTL`** | **REAL (Low)** | See T-1 below. Failing tests exist and are deliberately NOT committed. |
| 7 | `Governance._accrueDelegate` `:432-435` | concentration cap, `accrued * BPS <= capBps * p.snapshotTotal` | **TAINT** | No `block.timestamp` in the function; it reads `p.snapshotTotal` out of the tainted `proposals`. Dropped in ablation 2. VO-5. |
...
| 9 | `Governance.commitVote` `:358` | `block.timestamp < p.commitDeadline` — commit-phase membership | **DESIGN** | The strict `<` is the exact complement of the reveal family's `>=`. `::test_commitDeadline_partitionsExactly`. |
...
| 11 | `Governance.setDelegate` `:515` | `_isSettled(proposals[activePid].status)` — no delegation changes mid-proposal | **TAINT** | Reaches the clock only through `_refreshStatus` (`:606`). Dropped in ablation 2. |
...
| 14 | `Governance.revealDelegated` `:405-406` | reveal-phase membership for the delegation crank | **DESIGN** | The same guard as row 4, character for character. Same tests. |
...
| 17 | `Governance._isSettled` `:634` | `s == Defeated \|\| s == Executed \|\| s == Expired` | **TAINT** | A `pure` function over an enum — the clearest single proof that the detector is reporting argument taint rather than logic. Dropped in ablation 2. |
...
| 26 | `Governance._refreshStatus` `:627` | `block.timestamp > p.expiresAt` — lazy transition Passed → Expired | **DESIGN** | The same strict `>` as `markExpired`. Crucially `hasPendingExecution` does NOT depend on this crank having run — it re-derives expiry from the clock — so a stale `Passed` status never traps an exit. Same test as row 5. |
| 27 | `Governance.finalize` `:529` | `block.timestamp >= p.revealDeadline` — the tally may begin | **DESIGN** | Exact complement of the reveal family's `<`. `::test_revealDeadline_partitionsExactly`. |
| 28 | `Governance.hasPendingExecution` `:648, :651` | **the Mode I / Mode F exit seam** — `>= commitDeadline` while Active, `<= expiresAt` while Passed | **DESIGN** | The `<=` matches `execute`'s `<=` exactly, which is what makes EE-10 true. The `>= commitDeadline` start is VO-8: the outcome begins leaking on-chain at reveal, so exits after that must be forward-priced, and Mode F is deliberately OFF for the whole commit phase — the C-5 note above `Governance.revealVote` documents that seam. `::test_expiresAt_partitionsExactly_executeVsMarkExpired` and `::test_commitDeadline_partitionsExactly`. |
| 29 | `Governance.execute` `:591, :592` | timelock elapsed, and execution window not yet over | **DESIGN** | `>= executableAt` and `<= expiresAt`. `::test_executableAt_boundary` and `::test_expiresAt_partitionsExactly_executeVsMarkExpired`. |
...
### T-1 — `applyStandingDefault`'s TTL is measured at apply time (REAL, Low) — **FIXED on `protocol/main`**

> **Status, 2026-09-02.** `protocol/main` @ `bab5ee90` has landed **fix shape 1 below** as
> commit `bf34b1ff`: `COMMIT_HARD_CAP` is now `DEFAULT_TTL - 1` rather than `30 days`, so the
> dead configuration is unrepresentable. Regression `AuditStandingDefaultTtlVsCommit.t.sol`.
> The analysis below is retained as the record of the finding and its evidence; every sentence
> in it describing the validator in the present tense describes the tree **before** that commit.
> The disposition is unchanged (REAL, Low) — only its status moved from open to fixed.

```solidity
// Governance.applyStandingDefault:490-492
require(
    d.set && d.setAt < p.createdAt && block.timestamp <= d.setAt + DEFAULT_TTL, DefaultUnavailable()
);
```

`applyStandingDefault` is callable only during the reveal phase (`:478-479`), i.e. no earlier than
...
`_validateConfig` bounded `cfg.commitDuration` to `[1 hours, COMMIT_HARD_CAP]` with
`COMMIT_HARD_CAP = 30 days`, and **never related it to `DEFAULT_TTL = 72 hours`** (as of `52d10aee`;
since fixed on `main` — see the status note above). So a vault
registered with `commitDuration >= 72 hours` — legal until `bf34b1ff`, silent, no event, chosen by the creator at
`registerVault` and changeable later by RuleChange (`configOf[p.vault] = newCfg`, `Governance.execute:600`) — has
every standing default provably expired before its reveal window opens. VO-3 was dead for that
vault (unrepresentable since `bf34b1ff`), and the validator whose stated purpose is to stop a config disabling its own defences (its
own M-6 comment, `Governance._validateConfig:244-253`) did not object.
...
are refreshed. What was nowhere stated — docs, NatSpec or test — was that the window is
`72h - commitDuration`, and that it was zero at `commitDuration >= 72h`. **Both halves are now
stated on `main`:** `bf34b1ff` added a `@dev T-1` NatSpec block above
`Governance.setStandingDefault` naming `DEFAULT_TTL - cfg.commitDuration` as the usable window,
and a second above `applyStandingDefault`, plus `AuditStandingDefaultTtlVsCommit.t.sol`. **T-1b survives by design** — the window is
still shortened by `commitDuration`, and a 1-second window remains representable; only the total
kill is gone. **Not reachable at launch:**
...
**Fix shape (deliberately not applied on this audit branch; `main` has since applied shape 1).**
...
2. `Governance.applyStandingDefault:491`: compare against the proposal rather than the clock —
