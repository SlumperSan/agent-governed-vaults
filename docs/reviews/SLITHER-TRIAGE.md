...
done separately and mutation-verified at `c3c789ba` (an ancestor of `52d10aee`) against
`test/audit/AuditLookThroughReadOnlyReentrancy.t.sol` and
`test/audit/AuditReentrancyGuardCoverage.t.sol`.** In both, internal accounting is measured across a full external call
