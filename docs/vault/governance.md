...
all. So the fastest round this contract can run, using the values these configs carry, is
**1 h commit + 1 h reveal + 0 timelock = 2 h from `propose` to the earliest `execute`**. Going lower
needs a contract change, not a config edit, and `Governance.sol` is a **singleton shared by every
vault** — so that change is not scoped to one vault either.
