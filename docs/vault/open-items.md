...
The live to-do list between now and a meaningful `v1.0.0-launch-candidate`. Everything that keeps [[current-state]] at NO-GO for **Base mainnet**, plus the accepted residuals. The protocol is built for Arc but has no mainnet deployment anywhere today — a prior deployment on a since-abandoned chain was fully exited on 2026-09-18 and holds nothing, so none of these items was closed by that deployment either. The two gates at the top of the list (soak, canary) have no current evidence on any chain: the 5/5 soak run and the canary observation alongside it are real and are recorded in [[../SOAK-REPORT]], but they were earned against superseded bytecode and have not been re-run.
