...
- **Exit fee:** `exitFeeMaxBps = 50`, decay 604,800 s (7 days) — the values `base-mainnet.json` deploys. 50 bps was exercised end-to-end in the soak. This line previously said 302,400 s (3.5 days); that decay was only ever run by the 25 bps soak drill vaults (`scripts/soak/soak-vaults.json`), while the 50 bps smoke vault ran 604,800 s. 3.5 d vs 7 d is an open owner launch-parameter decision; until it is made the doc states what deploys, pinned by `scripts/test/config-doc-truth.test.mjs`.
