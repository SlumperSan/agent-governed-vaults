...
- [[c6-oracle-byzantine]]: **the `allowedOracles_` oracle-gate (PR #50).** The bespoke
  ...
  **reverts if the `BLESSED_ORACLES` allowlist is empty**, preventing accidental unsafe deploys. Gate 0
  ...
  owner attestation. Any mainnet deploy must still populate the allowlist with real addresses — readable on-chain via `factory.isAllowedOracle(...)` and `factory.oracleAllowlistEnforced()` once a deployment exists; a
