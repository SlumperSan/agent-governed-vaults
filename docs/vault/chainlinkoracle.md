...
- **`DeployChainlinkOracle.s.sol`**: deployment script; populates the chain's feed config at runtime (`base-mainnet.json` for Base). Its `requiresSequencerUptimeFeed(chainId)` is the deploy-time sequencer enforcement, and it is an ENUMERATION of exempt chains rather than a universal: currently a local node and Base Sepolia. Arc has no Chainlink L2 sequencer uptime feed either — it is an L1, not a rollup — but that enumeration has to be widened deliberately, in a reviewed change, before a deploy there; it is not exempt by default. On an exempt chain there is no deploy-time refusal and no price-time gate, so nothing enforces the feed there at all.
