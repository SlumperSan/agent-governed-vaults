...
Count toward tally, and **never toward quorum** (VO-2/K-3) - they touch neither `revealedWeight` nor `revealedVoterCount` in any regime. Below `SIGNER_REGIME_BELOW` they DO bear on passage: `finalize`'s sub-five branch tests FOR stake, so a Rebalance can pass on a >50% pre-declared-default majority with **zero live reveals** (demonstrated by `test_defaultsStillCarrySubFiveWithZeroLiveReveals` in `contracts/test/audit/AuditDelegatedQuorum.t.sol`). **Cranked delegations do NOT share that**: VO-2b subtracts `delegatedForWeight[pid]` out of both sub-five stake terms, because a default is the member's own direction while a crank is the delegate's. `Governance.finalize` names this asymmetry as intended, not overlooked — defaults are Rebalance-only, must predate the proposal, and only ever widen the passing set. Expire 72h after being set (`DEFAULT_TTL = 72 hours`); valid
