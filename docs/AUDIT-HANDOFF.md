...
`0x03E121e18c68B48B84a60D8F93BcD7D5be31ee38` holding 0.001980484 WETH (`assetBalance`), a priced position rather than cash, a priced
position rather than cash. This sentence said the deployment held
...
| `oracle/vendor/` (TickMath, FullMath) | ~170 | vendored Uniswap math under original licenses (GPL-2.0-or-later / MIT) | High: vendored; the license mix against the repository's own MIT (BUSL-1.1 until 2026-09-05) is an owner decision, not a question for the technical audit |
...
445 tests (2026-09-03), incl. these invariant suites (256 runs × 16k calls each):
...
```bash
cd contracts && forge build && forge test -vvv     # 445 tests (2026-09-03)
