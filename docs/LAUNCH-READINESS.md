...
| 6 | Canary operational | **STALE** — unblocked; re-run with the soak (needs testnet key) | Ran throughout the soak; every transition it ever recorded reconciles to a specific drill action, including dynamically discovering two new vaults on its own (SOAK-REPORT §6). That evidence came from watching the SUPERSEDED contracts, so it re-earns this row alongside the gate-3 re-run. **It is no longer true that "the canary is read-only and unchanged": running it against the pivoted deployment during the gate-7 restore drill found `oracle-freshness` permanently BLIND** — it read `assetConfig`, retired with the custom aggregator, so it reverted every poll on every asset and parked in DEGRADED, i.e. silent because it could not see (RESTORE-DRILL.md §5). The signal is now modelled on the deployed `ChainlinkOracle`: `priceWad` (which every oracle model implements) for the freeze alarm, feed `updatedAt` vs the configured heartbeat for the early warning, a named cause per `priceWad` reject branch, and the L2 sequencer gate as its own key. `ORACLE_MIN_MARGIN` → `ORACLE_MIN_HEADROOM_BPS` (2500). The fixtures now model the deployed oracle and `test/abis.test.mjs` checks the canary's oracle table against the COMPILED artifact, so this class of blindness fails in CI rather than on-chain. **None of that earns the gate** — a fixed signal is not a run: this row still needs the key and the live re-run. |
