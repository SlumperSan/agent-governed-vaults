...
>
> **What still blocks GO — all operational, all need a funded Base Sepolia key:** gates **2/3/6** (testnet full-lifecycle + soak + canary must be **re-run on the pivoted tree** — the Sepolia oracle config now exists and is on-chain-verified, PR #64, so this is unblocked and just needs a key), and gate **7** (one recorded restore drill, ~30 min, no keys). **Nothing security-side remains.**
>
> ---
...
2. **One recorded restore drill (gate 7)** — ~30 min, no keys, doable any time.
