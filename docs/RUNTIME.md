...
|---|---|---|---|
| `RPC_URL` | ✅ | n/a | Base HTTP RPC endpoint |
| `FACTORY_ADDRESS` | ✅ | n/a | VaultFactory |
| `OPERATOR_REGISTRY_ADDRESS` | ✅ | n/a | OperatorRegistry |
| `SUBVAULT_REGISTRY_ADDRESS` | ✅ | n/a | SubVaultRegistry |
| `GOVERNANCE_ADDRESS` | ✅ | n/a | Governance |
| `FEE_ENGINE_ADDRESS` | | n/a | FeeEngine. The **zero address counts as unset** (it is a valid 20-byte address and truthy, so a placeholder would otherwise suppress the warning below). **Unset is almost always a mistake**: every deploy script deploys one. Without it `FeeAssessed` / `FeeCredited` / `FeesClaimed` are not indexed and the indexer logs a `indexer.feeEngine.unset` warning at startup. Take it from `singletons.FeeEngine` in `contracts/config/deployments/<chain>.json`. |
| `ADAPTER_ADDRESSES` | | n/a | Comma-separated execution adapters. Polled unconditionally and **exempt from the 64-adapter discovery ceiling**, so an adapter you name here can never be crowded out by adapters an attacker stood up first (`createVault` is permissionless and `allowedAdapters` is caller-supplied). Adapters are a **per-vault** choice (`docs/DEPLOYMENT.md` "Execution adapters (per-vault)"), so their distinct count scales with creators. Name yours rather than relying on discovery. Unset = discovery only; the indexer logs `indexer.adapterCap.hit` once per batch when it declines one. |
| `CHAIN_ID` | | `8453` | `8453` Base, `84532` Base Sepolia |
| `CHAIN_NAME` | | `base` | label for the viem chain |
| `START_BLOCK` | | `0` | deploy block: skip empty history on a cold start. **`0` means "index from genesis"** — on Base or Base Sepolia that is tens of millions of blocks, and a run left at the `.env.example` placeholder measurably took ~2 hours to catch up against a 5-minute soak deadline. Take it from `startBlock` (or `deployBlock`) in `contracts/config/deployments/<chain>.json`, not from memory. The indexer logs `indexer.startBlock.unsetOnFresh` and a `⚠` line at startup if it is left at 0 on a fresh snapshot, so this is never silent — but the loud warning still costs the catch-up time; fix it before starting, don't wait for the log. **`scripts/soak/run-soak.ps1` sets this for you**, defaulting it from the same address book's `startBlock`/`deployBlock` and refusing to start at all if that record has no usable block — it never falls back to 0. |
...
| Var | Required | Default | Meaning |
|---|---|---|---|
| `FACILITATOR_I_UNDERSTAND_THIS_SPENDS_FUNDS` | ✅ | n/a | must be exactly `yes`; startup refuses otherwise |
| `SETTLER_KEYSTORE` | ✅ | n/a | path to a V3 keystore file (e.g. `~/.foundry/keystores/<name>`) |
| `SETTLER_KEYSTORE_PASSWORD` | ✅ | n/a | decrypted in-process; the key never enters the environment |
...
| `VAULTS` | | n/a | explicit vault list, to run without an indexer |
| `OPERATOR_REGISTRY_ADDRESS` | | n/a | enables the fee-routing signal |
...
| `PAGE_WEBHOOK_URL` | | n/a | POST one JSON body per **PAGE-tier** transition: the wake-a-human set (docs/CANARY.md §5.3) |
| `LOG_WEBHOOK_URL` | | n/a | POST one JSON body per **LOG-tier** transition: everything else |
| `ALERT_WEBHOOK_URL` | | n/a | back-compat fallback for whichever of the two above is unset; set only this and behaviour is pre-tiering |
| `DEADMAN_PING_URL` | | n/a | off-host dead-man's switch, `GET` once per successful sweep that watched **at least one vault** (docs/CANARY.md §5.3) |
...
| `ORACLE_FEED_CADENCE_SECONDS` | | n/a | `ChainlinkOracle` only: `addr:seconds` feed cadences that drive the derived early-warning bar (docs/CANARY.md §3a) |
