...
- **Arc TESTNET is not a dry run, and that is measured.** Chain 5042002 carries the USDC predeploy,
  ...
  asset and fails closed without one, so a deploy there could only stand up against mocks. Base
  Sepolia remains the functional testbed.
- **The prior deployment on another chain was wound down on 2026-09-18.** Both vaults were fully
  exited by their sole holder and read `totalShares() == 0`, `navWad() == 0`, `holderCount() == 0`;
  nothing of anyone's remains in them. Its configuration and deployment records have been removed
  from this repository, and the guard that required the copy to describe that deployment
  is retired with them, its premise having gone.
...
1. ~~**Resume the smoke lifecycle**~~: **DONE, re-run on the CURRENT tree 2026-09-03, and it
   ...
   **What it does NOT prove**, stated so the row is not over-read: the sequencer guard still has
   ...
   adapter. Those are the soak's job, below. The sequencer guard has still never had a real
   first run, and Arc will not give it one: Arc is an L1, Chainlink publishes no sequencer uptime
   feed for it, and there is nothing to wire. A first real execution waits for a chain that
   publishes one.
...
3. ~~**Recorded restore drill** (gate 7)~~ **DONE. Gate 7 is GO as of 2026-09-02 and nothing here
   ...
   **steps 1 and 6 literal** (`docker compose stop/start`). This was the one condition the row was
...
- **The operator needs ≥5% of eligible stake permanently, or loses the right to propose anything,
  ...
  not chosen*; "zero capital cost to the operator" is false. Say *low*, and state the number.
- **Below 5%, the operator cannot withdraw *anything* while a non-creator member remains.**
  ...
  share included). The same passive dilution that removes proposal rights also freezes the
  remaining capital. By design, not a bug (the gate binds creator *action*, CM-2), and recoverable
  via a top-up, a member exit, or the last member leaving. Pinned in
  `test/audit/AuditCreatorGateTraps.t.sol`; the arithmetic is Finance's (`Operator Capital
  Requirement.md`, "Dilution is passive, and the gate math is one-directional").
...
- **The oracle is single-provider Chainlink.** Heartbeat + sane-price band + sequencer gate are the
  ...
  the basket is cirBTC alone: every ETH-named token with a Uniswap v3 pool on Arc holds under $452
  of depth, and Circle mints no cirETH. The claim is about DEPTH rather than existence — ETH-named
  tokens are present on Arc and worthless — and `docs/evidence/arc-mainnet-survey.json` carries the
  scan it rests on. A feed deprecation
