...
still `'oracle-freshness'`, unchanged across the post-pivot rename), and `depeg-reference`.
`LOG_WEBHOOK_URL` receives every other transition: recoveries, every DEGRADED / DETECTOR BROKEN
line, the harmless half of `feed-identity`, and `operator-power`'s early-warning bar. `ALERT_WEBHOOK_URL` remains the backwards-compatible fallback used for whichever of
...
`npm run test:backend` includes `packages/canary/test/*.test.mjs` — 318 tests, every one with a
